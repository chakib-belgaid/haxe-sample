...
Main.main = function() 
  ...
  local benchname = args[0];
  local wamrup;
  local targs1 = __lua_PairTools.copy(_G.arg);
  local tab1 = __lua_PairTools.copy(targs1);
  local length1 = nil;
  local args1;
  if (length1 == nil) then 
    length1 = _hx_table.maxn(tab1);
    if (length1 > 0) then 
      local head1 = tab1[1];
      _G.table.remove(tab1, 1);
      tab1[0] = head1;
      args1 = _hx_tab_array(tab1, length1);
    else
      args1 = _hx_tab_array({}, 0);
    end;
  else
    args1 = _hx_tab_array(tab1, length1);
  end;
  if (args1[1] ~= nil) then 
    local targs2 = __lua_PairTools.copy(_G.arg);
    local tab2 = __lua_PairTools.copy(targs2);
    local length2 = nil;
    local args2;
    if (length2 == nil) then 
      length2 = _hx_table.maxn(tab2);
      if (length2 > 0) then 
        local head2 = tab2[1];
        _G.table.remove(tab2, 1);
        tab2[0] = head2;
        args2 = _hx_tab_array(tab2, length2);
      else
        args2 = _hx_tab_array({}, 0);
      end;
    else
      args2 = _hx_tab_array(tab2, length2);
    end;
    wamrup = Std.parseInt(args2[1]);
  else
    wamrup = 5;
  end;
  local targs3 = __lua_PairTools.copy(_G.arg);
  local tab3 = __lua_PairTools.copy(targs3);
  local length3 = nil;
  local args3;
  if (length3 == nil) then 
    length3 = _hx_table.maxn(tab3);
    if (length3 > 0) then 
      local head3 = tab3[1];
      _G.table.remove(tab3, 1);
      tab3[0] = head3;
      args3 = _hx_tab_array(tab3, length3);
    else
      args3 = _hx_tab_array({}, 0);
    end;
  else
    args3 = _hx_tab_array(tab3, length3);
  end;
  local execution = Std.parseInt(args3[2]);
  local targs4 = __lua_PairTools.copy(_G.arg);
  local tab4 = __lua_PairTools.copy(targs4);
  local length4 = nil;
  local args4;
  if (length4 == nil) then 
    length4 = _hx_table.maxn(tab4);
    if (length4 > 0) then 
      local head4 = tab4[1];
      _G.table.remove(tab4, 1);
      tab4[0] = head4;
      args4 = _hx_tab_array(tab4, length4);
    else
      args4 = _hx_tab_array({}, 0);
    end;
  else
    args4 = _hx_tab_array(tab4, length4);
  end;
  if (args4[2] ~= nil) then 
    local targs5 = __lua_PairTools.copy(_G.arg);
    local tab5 = __lua_PairTools.copy(targs5);
    local length5 = nil;
    local args5;
    if (length5 == nil) then 
      length5 = _hx_table.maxn(tab5);
      if (length5 > 0) then 
        local head5 = tab5[1];
        _G.table.remove(tab5, 1);
        tab5[0] = head5;
        args5 = _hx_tab_array(tab5, length5);
      else
        args5 = _hx_tab_array({}, 0);
      end;
    else
      args5 = _hx_tab_array(tab5, length5);
    end;
    Std.parseInt(args5[2]);
  else
    execution = 50;
  end;
  if (benchname ~= nil) then 
    if (benchname == "intArithmetic") then 
      Main.__intArithmetic(wamrup, execution);
    else
      if (benchname == "doubleArithmetic") then 
        Main.__doubleArithmetic(wamrup, execution);
      else
        if (benchname == "trig") then 
          Main.__trig(wamrup, execution);
        else
          __haxe_Log.trace("error module not found ", _hx_o({__fields__={fileName=true,lineNumber=true,className=true,methodName=true},fileName="Main.hx",lineNumber=32,className="Main",methodName="main"}));
        end;
      end;
    end;
  else
...
Main.__intArithmetic = function(wamrup,execution) 
  local i = 0;
  __haxe_Log.trace("--++beginwarmup", _hx_o({__fields__={fileName=true,lineNumber=true,className=true,methodName=true},fileName="Main.hx",lineNumber=43,className="Main",methodName="__intArithmetic"}));
  while (i < wamrup) do 
    Main.bench:intArithmetic(Main.intMax);
    i = i + 1;
  end;
  i = 0;
  __haxe_Log.trace("++--endwarmup", _hx_o({__fields__={fileName=true,lineNumber=true,className=true,methodName=true},fileName="Main.hx",lineNumber=49,className="Main",methodName="__intArithmetic"}));
  while (i < execution) do 
    Main.bench:intArithmetic(Main.intMax);
    i = i + 1;
  end;
end
Main.__doubleArithmetic = function(wamrup,execution) 
  local i = 0;
  __haxe_Log.trace("--++beginwarmup", _hx_o({__fields__={fileName=true,lineNumber=true,className=true,methodName=true},fileName="Main.hx",lineNumber=58,className="Main",methodName="__doubleArithmetic"}));
  while (i < wamrup) do 
    Main.bench:doubleArithmetic(Main.floatMin, Main.floatMax);
    i = i + 1;
  end;
  i = 0;
  __haxe_Log.trace("++--endwarmup", _hx_o({__fields__={fileName=true,lineNumber=true,className=true,methodName=true},fileName="Main.hx",lineNumber=64,className="Main",methodName="__doubleArithmetic"}));
  while (i < execution) do 
    Main.bench:doubleArithmetic(Main.floatMin, Main.floatMax);
    i = i + 1;
  end;
...
Main.__trig = function(wamrup,execution) 
  local i = 0;
  __haxe_Log.trace("--++beginwarmup", _hx_o({__fields__={fileName=true,lineNumber=true,className=true,methodName=true},fileName="Main.hx",lineNumber=73,className="Main",methodName="__trig"}));
  while (i < wamrup) do 
    Main.bench:trig(Main.trigMax);
    i = i + 1;
  end;
  i = 0;
  __haxe_Log.trace("++--endwarmup", _hx_o({__fields__={fileName=true,lineNumber=true,className=true,methodName=true},fileName="Main.hx",lineNumber=79,className="Main",methodName="__trig"}));
  while (i < execution) do 
    Main.bench:trig(Main.trigMax);
    i = i + 1;
  end;
...
Bencher.new = function() 
  local self = _hx_new(Bencher.prototype)
  Bencher.super(self)
  return self
end
Bencher.super = function(self) 
end
Bencher.prototype = _hx_a();
Bencher.prototype.intArithmetic = function(self,intMax) 
  local intResult = 1;
  local myIterator_min = 0;
  local myIterator_max = intMax;
  while (myIterator_min < myIterator_max) do 
    myIterator_min = myIterator_min + 1;
    local i = myIterator_min - 1;
...
Bencher.prototype.doubleArithmetic = function(self,floatMin,floatMax) 
  local floatResult = floatMin;
  local myIterator_min = floatMin;
  local myIterator_max = floatMax;
  while (myIterator_min < myIterator_max) do 
    myIterator_min = myIterator_min + 1;
    local i = myIterator_min - 1;
...
Bencher.prototype.trig = function(self,trigMax) 
  local sine = 0.0;
  local cosine = 0.0;
  local tangent = 0.0;
  local logarithm = 0.0;
  local squareRoot = 0.0;
  local i = 1;
  while (i < trigMax) do 
    sine = _G.math.sin(i);
    cosine = _G.math.cos(i);
    tangent = _G.math.tan(i);
    logarithm = _G.math.log(i);
    squareRoot = _G.math.sqrt(i);
    i = i + 1;
  end;
end
